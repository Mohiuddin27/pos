...
<style>
    ...
    <main class="content">
        <div class="container-fluid p-0">
            ...
            <h1 class="h3 mb-3"><strong>Product Brand</h1>
                                ...
                                <div class="card">
                                    <div class="card-header">
                                        <div class="d-flex justify-content-between">
                                            <h4 class="card-title">All Product Brand</h4>
                                            <div></div>
                                            <div>
                                                {{-- @if(Auth::guard('admin')->user()->can('party.type.create')) --}}
                                                <a href="#productBrand-modal" data-bs-toggle="modal"
                                                    class="btn btn-primary btn-sm mb-3"><i class="fa fa-plus-circle"
                                                        aria-hidden="true"></i> Add New Product Brand</a>
                                                {{-- @endif --}}
                                            </div>
                                        </div>
                                       <br>
                                                            ...
                                                            <div class="dropdown partyy text-center">
                                                                ...
                                                                <ul class="dropdown-menu" style="text-align: center" aria-labelledby="dropdownMenuButton1">
                                                                  <li>  <a class="btn btn-warning btn-sm dropdown-item text-center" edit_id="{{ $data->id }}"
                                                                    data-bs-toggle="modal" id="edit_product_brand"><i
                                                                        class="fa fa-pencil-square-o  text-dark"
                                                                        style="font-size:20px" aria-hidden="true"> Edit</i>
                                                                </a></li>
                                                                  <li> <a class="btn btn-light btn-sm dropdown-item text-center" id="delete_product_brand"
            ...
            <div id="productBrand-modal" class="modal fade">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class="mess"></div>
                            <form id="add_product_brand_form" action="" method="POST">
                                @csrf
                                <div class="form-group">
                                    <label for="" class="mb-2">Product Brand name</label>
            ...
            {{-- //edit party type --}}
            <div id="editProductBrand-modal" class="modal fade">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal-header">
...
<script>
    ...
    $(document).on('submit', 'form#add_product_brand_form', function(e) {
        e.preventDefault();
        var name = $("#name").val();
        var fd = new FormData();
        fd.append('name',name);
        ...
        $.ajax({
            ...
            success: function(res) {
                if (res.status == 200) {
                    $('#productBrand-modal').modal('hide');
                    Swal.fire(
                        'Added',
                        'Product Brand Added Successfully!',
                        'success'
                    )
                    setTimeout(function() {
    ...
    //edit party type
    $(document).on('click', '#edit_product_brand', function(e) {
        e.preventDefault();

        let edit_id = $(this).attr('edit_id');
        $.ajax({
            url: 'product-brand/edit/' + edit_id,
            success: function(data) {
                $('#editProductBrand-modal input[id="edit_id"]').val(data.id);
                $('#editProductBrand-modal input[id="edit_name"]').val(data.name);
                $('#editProductBrand-modal select[id="edit_status"]').val(data.status);


                $('#editProductBrand-modal').modal('show');
            }
        })
    });
   ...
   $(document).on('submit', 'form#updateData', function(e) {
        ...
        $.ajax({
            ...
            success: function(res) {
                if (res.status == 200) {
                    $('#editProductCategory-modal').modal('hide');
                    Swal.fire(
                        'Update',
                        'Product Brand Update Successfully!',
                        'success'
                    )
                    setTimeout(function() {
                        window.location.reload();
    ...
    $(document).on('click', 'a#delete_product_brand', function(e) {
        e.preventDefault();
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: 'product-brand-temporary-delete/' + id,
                    success: function(data) {
                        Swal.fire(
                            'Delete',
